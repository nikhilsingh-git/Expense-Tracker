...
const EditIncome = () => {
  ...
  useEffect(() => {
    ...
    date.current.value = singleIncome?.date
      ? new Date(singleIncome?.date).toISOString().split("T")[0]
      : "";
    ...
    <>
      <div className="text-gray-300">
        <div className="text-center mt-15 ">
          ...
          <p>
            Update your income details quickly and keep your financial records
            accurate.
          </p>
        </div>
        <div className="w-200 m-auto mt-5 ">
          <form onSubmit={(e) => incomeFormSubmit(e)}>
            <div className="flex flex-col font-sans font-medium ms-5 py-10 ">
              <label className="my-2 font-sans font-medium text-md">
                Add Income (₹)
              </label>
              <input
                type="text"
              ...
              <div className="flex justify-around">
                <button
                  type="button"
                  className="bg-transparent border w-45 h-11 rounded-md cursor-pointer"
                >
                  Back
                </button>
                <button
                  type="submit"
                  className="bg-emerald-500 w-45 h-11 rounded-lg border cursor-pointer"
                >
                  Update
                </button>
              </div>
